...
3. **researcher + copywriter (caption = тот же шаг) + image-prompter:** модель **`gemini-3.7-flash-high`** (роль: `gemini-3.7-flash`). Не inherit модели Director. Артефакты обязаны нести `written_by: gemini`. Не Kie, не Composio, не face-check.
...
# researcher / copywriter / image-prompter:
...
python scripts/pipeline_gate.py --workspace . record-dispatch --step image-prompter --via 'Task(generalPurpose)' --model gemini-3.7-flash-high

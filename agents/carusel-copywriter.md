...
**Модель:** `model="inherit"` (наследует Gemini родителя `gemini-3.8-flash` + `reasoning_effort=low`). high только если Владимир явно переопределил. Слайды, caption RU+EN, хуки, CTA — только Gemini. НЕ передавать slug `gemini-3.8-flash` в Task.
